...
bool ompi_request_state_ok(ompi_request_t *req)
{
    ...
    if( OPAL_UNLIKELY(ompi_comm_is_revoked(req->req_mpi_object.comm) && req->req_type == OMPI_REQUEST_PML && !ompi_request_tag_is_ft(req->req_tag)) ) {
        ...
        opal_output_verbose(10, ompi_ftmpi_output_handle,
                            "%s ompi_request_state_ok: %p (peer %d, tag %d) is on communicator %s(%d) that has been revoked!",
                            OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), (void*)req, req->req_peer, req->req_tag,
    ...
    if( OPAL_UNLIKELY(ompi_comm_coll_revoked(req->req_mpi_object.comm) &&
        ...
        opal_output_verbose(10, ompi_ftmpi_output_handle,
                            "%s ompi_request_state_ok: Request %p (peer %d) is part of a collective (tag %d), and some process died. (mpi_source %3d)",
                            OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), (void*)req, req->req_peer, req->req_tag, req->req_status.MPI_SOURCE );
        goto return_with_error;
    }
    ...
    if( OPAL_UNLIKELY(MPI_ANY_SOURCE == req->req_peer && !ompi_comm_is_any_source_enabled(req->req_mpi_object.comm)) ) {
        if( !ompi_request_tag_is_ft(req->req_tag) ) {
            ...
            opal_output_verbose(10, ompi_ftmpi_output_handle,
                                "%s ompi_request_state_ok: Request %p (peer %d, tag %d) in comm %s(%d) peer ANY_SOURCE %s!",
                                OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), (void*)req, req->req_peer, req->req_tag,
                                req->req_mpi_object.comm->c_name, req->req_mpi_object.comm->c_contextid,
    ...
    if( OPAL_UNLIKELY(!ompi_comm_is_proc_active(req->req_mpi_object.comm, req->req_peer,
        ...
        opal_output_verbose(10, ompi_ftmpi_output_handle,
                            "%s ompi_request_state_ok: Request %p (peer %d, tag %d) in comm %s(%d) mpi_source %3d failed - Ret %s",
                            OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), (void*)req, req->req_peer, req->req_tag,
                            req->req_mpi_object.comm->c_name, req->req_mpi_object.comm->c_contextid,
    ...
    OPAL_OUTPUT_VERBOSE((100, ompi_ftmpi_output_handle,
                         "%s ompi_request_state_ok: Request %p (peer %d tag %d) is still ok. (mpi_source %3d)",
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), (void*)req, req->req_peer, req->req_tag, req->req_status.MPI_SOURCE ));
