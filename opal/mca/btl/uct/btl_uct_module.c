                                                   ...
                                                   opal_proc_t *proc)
{
    ...
    do {
        rc = opal_hash_table_get_value_uint64(&uct_module->id_to_endpoint, (intptr_t) proc,
                                              (void **) &ep);
        if (OPAL_SUCCESS == rc) {
            BTL_VERBOSE(
    ...
    if (false == uct_module->initialized) {
        ...
        if (am_tl) {
            ...
            rc = opal_free_list_init(&uct_module->eager_frags, sizeof(mca_btl_uct_base_frag_t),
                                     opal_cache_line_size, OBJ_CLASS(mca_btl_uct_base_frag_t),
                                     btl->btl_eager_limit, opal_cache_line_size, 0, 1024, 64, NULL,
                                     0, uct_module->rcache, NULL, NULL);

            rc = opal_free_list_init(&uct_module->max_frags, sizeof(mca_btl_uct_base_frag_t),
                                     opal_cache_line_size, OBJ_CLASS(mca_btl_uct_base_frag_t),
                                     btl->btl_max_send_size, opal_cache_line_size, 0, 128, 8, NULL,
                                     0, uct_module->rcache, NULL, NULL);
        }
...
static int mca_btl_uct_del_procs(mca_btl_base_module_t *btl, size_t nprocs, opal_proc_t **procs,
                                 mca_btl_base_endpoint_t **peers)
{
    ...
    for (size_t i = 0; i < nprocs; ++i) {
        ...
        rc = opal_hash_table_get_value_uint64(&uct_module->id_to_endpoint, (intptr_t) procs[i],
                                              (void **) &ep);
        if (OPAL_SUCCESS != rc) {
            continue;
        }

        (void) opal_hash_table_remove_value_uint64(&uct_module->id_to_endpoint,
                                                   (intptr_t) procs[i]);
        OBJ_RELEASE(ep);
    }
...
mca_btl_uct_register_mem(struct mca_btl_base_module_t *btl,
 ...
 * a remote node.
 */
static int mca_btl_uct_deregister_mem(mca_btl_base_module_t *btl,
                                      mca_btl_base_registration_handle_t *handle)
{
    mca_btl_uct_module_t *uct_module = (mca_btl_uct_module_t *) btl;
    mca_btl_uct_reg_t *reg = (mca_btl_uct_reg_t *) ((intptr_t) handle
                                                    - offsetof(mca_btl_uct_reg_t, handle));
                        ...
                        mca_rcache_base_registration_t *reg)
{
    ...
    if (reg->access_flags
        & (MCA_BTL_REG_FLAG_REMOTE_READ | MCA_BTL_REG_FLAG_REMOTE_WRITE
           | MCA_BTL_REG_FLAG_REMOTE_ATOMIC)) {
        /* requested registration may be used by a remote process so go ahead and pack
         * the registration handle */
        ucs_status = uct_md_mkey_pack(uct_module->md->uct_md, uct_reg->uct_memh,
                                      uct_reg->handle.packed_handle);
