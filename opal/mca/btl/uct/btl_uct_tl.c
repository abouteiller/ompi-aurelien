...
found:
    BTL_PEER_ERROR(proc_opal, ("BTL UCT reported an error %d: %s", status, ucs_status_string(status)));
    if(NULL != module->uct_error_cb) {
        module->uct_error_cb(&module->super, MCA_BTL_ERROR_FLAGS_NONFATAL,
                             proc_opal,
                             "UCT ERROR was reported");
    }
    return UCS_OK;
}
...
static void mca_btl_uct_set_tl_rdma(mca_btl_uct_module_t *module, mca_btl_uct_tl_t *tl)
{
    ...
    if (MCA_BTL_UCT_TL_ATTR(tl, 0).cap.get.max_bcopy) {
        ...
        module->super.btl_get_local_registration_threshold = MCA_BTL_UCT_TL_ATTR(tl, 0)
                                                                 .cap.get.max_bcopy;
    } else {
        ...
        module->super.btl_get_alignment = opal_next_poweroftwo_inclusive(
            MCA_BTL_UCT_TL_ATTR(tl, 0).cap.get.min_zcopy);
    }
    ...
    module->super.btl_put_local_registration_threshold = MCA_BTL_UCT_TL_ATTR(tl, 0)
                                                             .cap.put.max_bcopy;
