...
static void errreg_cbfunc (pmix_status_t status,
                           ...
                           void *cbdata)
{
    opal_pmix4x_event_t *event = (opal_pmix4x_event_t*)cbdata;
...
int pmix4x_client_init(opal_list_t *ilist)
{
    ...
    if (0 == opal_pmix_base.initialized) {
        ...
        if (OPAL_SUCCESS != (dbg = opal_pmix_pmix4x_check_evars())) {
    ...
    if (NULL != ilist && 0 < (ninfo = opal_list_get_size(ilist))) {
        ...
        OPAL_LIST_FOREACH(ival, ilist, opal_value_t) {
            ...
            pmix4x_value_load(&pinfo[n].value, ival);
            ++n;
        }
    } else {
        pinfo = NULL;
    ...
    if (NULL != getenv(OPAL_MCA_PREFIX"orte_launch")) {
        ...
        mca_pmix_pmix4x_component.native_launch = true;
        opal_convert_string_to_jobid(&pname.jobid, mca_pmix_pmix4x_component.myproc.nspace);
    } else {
        /* we were launched by someone else, so make the
         * jobid just be the hash of the nspace */
        OPAL_HASH_JOBID(mca_pmix_pmix4x_component.myproc.nspace, pname.jobid);
    }
    ...
    pname.vpid = pmix4x_convert_rank(mca_pmix_pmix4x_component.myproc.rank);
    ...
    PMIx_Register_event_handler(NULL, 0, NULL, 0, pmix4x_event_hdlr, errreg_cbfunc, event);
...
{
    opal_pmix4x_event_t *ev = (opal_pmix4x_event_t*)cbdata;
    OPAL_PMIX_WAKEUP_THREAD(&ev->lock);
}

int pmix4x_client_finalize(void)
{
    pmix_status_t rc;
...
int pmix4x_tool_init(opal_list_t *info)
{
    pmix_info_t *pinfo;
    size_t ninfo, n;
    opal_pmix4x_jobid_trkr_t *job;
    ...
    if (NULL != info && 0 < (ninfo = opal_list_get_size(info))) {
        ...
        OPAL_LIST_FOREACH(val, info, opal_value_t) {
            ...
            pmix4x_value_load(&pinfo[n].value, val);
            ++n;
            ...
            if (0 == strcmp(val->key, OPAL_PMIX_TOOL_NSPACE)) {
                ...
                (void)strncpy(mca_pmix_pmix4x_component.myproc.nspace, val->data.string, PMIX_MAX_NSLEN);
            } else if (0 == strcmp(val->key, OPAL_PMIX_TOOL_RANK)) {
                pname.vpid = val->data.name.vpid;
                mca_pmix_pmix4x_component.myproc.rank = pname.vpid;
            }
        }
    } else {
        pinfo = NULL;
    ...
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
    rc = PMIx_tool_init(&mca_pmix_pmix4x_component.myproc, pinfo, ninfo);
    if (NULL != pinfo) {
        PMIX_INFO_FREE(pinfo, ninfo);
    }
    if (PMIX_SUCCESS != rc) {
        ret = pmix4x_convert_rc(rc);
        OPAL_ERROR_LOG(ret);
        return ret;
    }
    OPAL_PMIX_ACQUIRE_THREAD(&opal_pmix_base.lock);
...
int pmix4x_abort(int flag, const char *msg,
                 opal_list_t *procs)
{
    pmix_status_t rc;
    ...
    if (NULL != procs && 0 < (cnt = opal_list_get_size(procs))) {
        ...
        OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
            ...
            parray[n].rank = pmix4x_convert_opalrank(ptr->name.vpid);
            ++n;
        }
    }
...
int pmix4x_store_local(const opal_process_name_t *proc, opal_value_t *val)
{
    pmix_value_t kv;
    pmix_status_t rc;
    pmix_proc_t p;
    ...
    if (NULL != proc) {
        ...
        p.rank = pmix4x_convert_opalrank(proc->vpid);
    } else {
        /* use our name */
        (void)strncpy(p.nspace, mca_pmix_pmix4x_component.myproc.nspace, PMIX_MAX_NSLEN);
        p.rank = pmix4x_convert_opalrank(OPAL_PROC_MY_NAME.vpid);
    }

    PMIX_VALUE_CONSTRUCT(&kv);
    pmix4x_value_load(&kv, val);
...
{
    pmix4x_opcaddy_t *op = (pmix4x_opcaddy_t*)cbdata;

    OPAL_ACQUIRE_OBJECT(op);
    if (NULL != op->opcbfunc) {
        op->opcbfunc(pmix4x_convert_rc(status), op->cbdata);
    }
    OBJ_RELEASE(op);
}

int pmix4x_fence(opal_list_t *procs, int collect_data)
{
    pmix_status_t rc;
    opal_namelist_t *ptr;
    char *nsptr;
...
int pmix4x_fencenb(opal_list_t *procs, int collect_data,
...
int pmix4x_put(opal_pmix_scope_t opal_scope,
               opal_value_t *val)
{
    pmix_value_t kv;
    pmix_scope_t pmix_scope = pmix4x_convert_opalscope(opal_scope);
    pmix_status_t rc;
...
int pmix4x_get(const opal_process_name_t *proc, const char *key,
               opal_list_t *info, opal_value_t **val)
{
    ...
    opal_output_verbose(1, opal_pmix_base_framework.framework_output,
                        "%s pmix4x:client get on proc %s key %s",
    ...
    if (NULL == proc) {
        ...
        if (0 == strcmp(key, OPAL_PMIX_RANK)) {
            ...
            (*val)->key = strdup(key);
            (*val)->type = OPAL_INT;
            (*val)->data.integer = pmix4x_convert_rank(mca_pmix_pmix4x_component.myproc.rank);
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_SUCCESS;
        }
    }
    *val = NULL;
    ...
    if (NULL == proc) {
        (void)strncpy(p.nspace, mca_pmix_pmix4x_component.myproc.nspace, PMIX_MAX_NSLEN);
        p.rank = pmix4x_convert_rank(PMIX_RANK_WILDCARD);
    } else {
        if (NULL == (nsptr = pmix4x_convert_jobid(proc->jobid))) {
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
    ...
    if (PMIX_SUCCESS == rc) {
        ...
        if (OPAL_SUCCESS != (ret = pmix4x_value_unload(ival, pval))) {
            rc = pmix4x_convert_opalrc(ret);
        } else {
            *val = ival;
        }
...
int pmix4x_getnb(const opal_process_name_t *proc, const char *key,
                 opal_list_t *info,
                 opal_pmix_value_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == proc) {
        (void)strncpy(op->p.nspace, mca_pmix_pmix4x_component.myproc.nspace, PMIX_MAX_NSLEN);
        op->p.rank = pmix4x_convert_rank(PMIX_RANK_WILDCARD);
    } else {
        if (NULL == (nsptr = pmix4x_convert_jobid(proc->jobid))) {
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
        ...
        op->p.rank = pmix4x_convert_opalrank(proc->vpid);
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
    ...
    if (NULL != info && 0 < (op->sz = opal_list_get_size(info))) {
        ...
        OPAL_LIST_FOREACH(val, info, opal_value_t) {
            ...
            pmix4x_value_load(&op->info[n].value, val);
            ++n;
        }
    }
...
int pmix4x_publish(opal_list_t *info)
{
    pmix_info_t *pinfo;
    pmix_status_t ret;
    opal_value_t *iptr;
    ...
    if (0 < sz) {
        ...
        OPAL_LIST_FOREACH(iptr, info, opal_value_t) {
            ...
            pmix4x_value_load(&pinfo[n].value, iptr);
            ++n;
        }
    } else {
        pinfo = NULL;
    ...
    return pmix4x_convert_rc(ret);
}

int pmix4x_publishnb(opal_list_t *info,
                     opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    pmix_status_t ret;
    opal_value_t *iptr;
    ...
    if (0 < op->sz) {
        ...
        OPAL_LIST_FOREACH(iptr, info, opal_value_t) {
            ...
            pmix4x_value_load(&op->info[n].value, iptr);
            ++n;
        }
    }
...
int pmix4x_lookup(opal_list_t *data, opal_list_t *info)
{
    opal_pmix_pdata_t *d;
    pmix_pdata_t *pdata;
    pmix_info_t *pinfo = NULL;
    ...
    size_t cnt, n, sz = 0;
    opal_value_t *iptr;
    opal_pmix4x_jobid_trkr_t *jptr, *job;
    int ret;

    opal_output_verbose(1, opal_pmix_base_framework.framework_output,
                        "pmix4x:client lookup");
    ...
    if (PMIX_SUCCESS == rc) {
        ...
        OPAL_PMIX_ACQUIRE_THREAD(&opal_pmix_base.lock);
        OPAL_LIST_FOREACH(d, data, opal_pmix_pdata_t) {
            if (mca_pmix_pmix4x_component.native_launch) {
                /* if we were launched by the OMPI RTE, then
                 * the jobid is in a special format - so get it */
            ...
            job = NULL;
            OPAL_LIST_FOREACH(jptr, &mca_pmix_pmix4x_component.jobids, opal_pmix4x_jobid_trkr_t) {
            ...
            d->proc.vpid = pmix4x_convert_rank(pdata[n].proc.rank);
            if (OPAL_SUCCESS != (ret = pmix4x_value_unload(&d->value, &pdata[n].value))) {
                OPAL_ERROR_LOG(ret);
            }
        }
        OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
                      ...
                      void *cbdata)
{
    ...
    int rc;
    size_t n;
    opal_pmix4x_jobid_trkr_t *job, *jptr;
    ...
    rc = pmix4x_convert_rc(op->status);
    if (OPAL_SUCCESS == rc) {
        OPAL_PMIX_ACQUIRE_THREAD(&opal_pmix_base.lock);
        ...
        for (n=0; n < ndata; n++) {
            ...
            d->proc.vpid = pmix4x_convert_rank(data[n].proc.rank);
            d->value.key = strdup(data[n].key);
            rc = pmix4x_value_unload(&d->value, &data[n].value);
            if (OPAL_SUCCESS != rc) {
...
int pmix4x_lookupnb(char **keys, opal_list_t *info,
...
int pmix4x_unpublish(char **keys, opal_list_t *info)
{
    pmix_status_t ret;
    size_t ninfo, n;
    pmix_info_t *pinfo;
...
int pmix4x_unpublishnb(char **keys, opal_list_t *info,
                       opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    pmix_status_t ret;
    pmix4x_opcaddy_t *op;
...
int pmix4x_spawn(opal_list_t *job_info, opal_list_t *apps, opal_jobid_t *jobid)
{
    pmix_status_t rc;
    ...
    *jobid = OPAL_JOBID_INVALID;
    ...
    if (NULL != job_info && 0 < (ninfo = opal_list_get_size(job_info))) {
        ...
        OPAL_LIST_FOREACH(ival, job_info, opal_value_t) {
            ...
            pmix4x_value_load(&info[n].value, ival);
            ++n;
        }
    }
    ...
    OPAL_LIST_FOREACH(app, apps, opal_pmix_app_t) {
        ...
        if (0 < (papps[n].ninfo = opal_list_get_size(&app->info))) {
            ...
            OPAL_LIST_FOREACH(ival, &app->info, opal_value_t) {
                ...
                pmix4x_value_load(&papps[n].info[m].value, ival);
                ++m;
            }
        }
        ++n;
...
int pmix4x_spawnnb(opal_list_t *job_info, opal_list_t *apps,
                   opal_pmix_spawn_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL != job_info && 0 < (op->ninfo = opal_list_get_size(job_info))) {
        ...
        OPAL_LIST_FOREACH(info, job_info, opal_value_t) {
            ...
            pmix4x_value_load(&op->info[n].value, info);
            ++n;
        }
    }
    ...
    OPAL_LIST_FOREACH(app, apps, opal_pmix_app_t) {
        ...
        if (0 < (op->apps[n].ninfo = opal_list_get_size(&app->info))) {
            ...
            OPAL_LIST_FOREACH(info, &app->info, opal_value_t) {
                ...
                pmix4x_value_load(&op->apps[n].info[m].value, info);
                ++m;
            }
        }
        ++n;
...
int pmix4x_connect(opal_list_t *procs)
{
    pmix_proc_t *p;
    size_t nprocs;
    opal_namelist_t *ptr;
    ...
    OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
        ...
        p[n].rank = pmix4x_convert_opalrank(ptr->name.vpid);
        ++n;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
...
int pmix4x_connectnb(opal_list_t *procs,
                     ...
                     void *cbdata)
{
    ...
    OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
        ...
        op->procs[n].rank = pmix4x_convert_opalrank(ptr->name.vpid);
        ++n;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
...
int pmix4x_disconnect(opal_list_t *procs)
{
    size_t nprocs, n;
    opal_namelist_t *ptr;
...
int pmix4x_disconnectnb(opal_list_t *procs,
                        opal_pmix_op_cbfunc_t cbfunc,
                        void *cbdata)
{
    pmix4x_opcaddy_t *op;
...
int pmix4x_resolve_peers(const char *nodename,
                         opal_jobid_t jobid,
                         opal_list_t *procs)
{
    pmix_status_t ret;
    ...
    if (OPAL_JOBID_WILDCARD != jobid) {
        if (NULL == (nspace = pmix4x_convert_jobid(jobid))) {
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
        }
    } else {
    ...
    if (NULL != array && 0 < nprocs) {
        ...
        for (n=0; n < nprocs; n++) {
            ...
            /* if we don't already have it, add this to our jobid tracker */
            if (NULL == pmix4x_convert_jobid(nm->name.jobid)) {
                job = OBJ_NEW(opal_pmix4x_jobid_trkr_t);
                (void)strncpy(job->nspace, array[n].nspace, PMIX_MAX_NSLEN);
                job->jobid = nm->name.jobid;
                opal_list_append(&mca_pmix_pmix4x_component.jobids, &job->super);
            }
            nm->name.vpid = pmix4x_convert_rank(array[n].rank);
        }
        OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
    }
...
int pmix4x_resolve_nodes(opal_jobid_t jobid, char **nodelist)
{
    pmix_status_t ret;
    char *nsptr;
    ...
    if (NULL == (nsptr = pmix4x_convert_jobid(jobid))) {
        OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
        return OPAL_ERR_NOT_FOUND;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
...
int pmix4x_allocate(opal_pmix_alloc_directive_t directive,
                    opal_list_t *info,
...
int pmix4x_job_control(opal_list_t *targets,
